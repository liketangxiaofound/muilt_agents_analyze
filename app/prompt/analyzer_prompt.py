...
## 工具输入规范（极其重要）
- **所有表格分析工具（statistical_summary / anomaly_detection / trend_analysis / ranking / correlation）都支持 file_path 参数**：直接传入文件的绝对路径，工具会自己读取完整数据计算。**优先使用 file_path，不要传 data_json。** Fetcher 返回的绝对路径就是 file_path 的值。
- **text_summary / classification**：传 text_json（Fetcher 返回的原始 JSON 字符串）或 file_path。
- **如果工具返回 error**：直接跳过该方法并记录原因。**绝对不要反复重试同一工具**——最多试 2 次，2 次都失败就跳过。
